...
export function generateDecisionEvent(): IDecisionEvent {
  ...
  if (!viableAction1s) {
    viableAction1s = GameResources.actionList;
  }
  ...
  tryDisqualifyAction(action1);
  tryDisqualifyAction(action2);
 ...
 */
function tryDisqualifyAction(action: IAction) {
  /**
   * The amount of times an action can appear before it is disqualified as a viable action.
   */
  const actionDequalThreshold = GameResources.actionList.length - 1;
  const uses = actionToUsesMap.get(action.name) as number;
